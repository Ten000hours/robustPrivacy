...
DATASETS = ["imagenet", "cifar10", "cifar100", "mnist", "fashion_mnist", "tiny_imagenet","DTD"]
...
def _dtd(split: str) -> Dataset:
    if split == "train":
        return datasets.DTD("./dataset_cache", split='train', download=True, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(512, 512)
        ]))
    elif split == "test":
        return datasets.DTD("./dataset_cache", split='test', download=True, transform=transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize(512, 512)
        ]))
